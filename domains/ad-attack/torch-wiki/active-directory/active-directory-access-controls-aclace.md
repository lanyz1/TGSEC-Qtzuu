...
type: technique
tags: [access-control, active-directory, enumeration, reference-import, windows]
phase: enumeration
...
Attackers with misconfigured ACE permissions over AD objects can abuse rights like `GenericAll`, `GenericWrite`, `WriteDACL`, and `WriteOwner` to take control of user accounts, computers, or groups. By modifying ACEs, an attacker can grant themselves privileges such as the ability to reset passwords, add group members, or set a Service Principal Name (SPN) for kerberoasting. These attacks work because AD enforces resource access based on the DACL attached to each object, and any principal granted write access to that DACL can rewrite their own permissions.
...
- [[john]]
- [[mimikatz]]
- Also uses (no dedicated page yet): PowerView, SharpHound, PowerSploit
