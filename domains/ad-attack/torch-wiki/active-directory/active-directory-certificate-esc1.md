...
type: technique
tags: [active-directory, adcs, certificates, esc1, exploitation, reference-import, windows]
phase: exploitation
...
Attackers enroll in certificate templates where `ENROLLEE_SUPPLIES_SUBJECT` is set, allowing them to specify an arbitrary Subject Alternative Name (SAN) during enrollment. By requesting a certificate with a domain admin's UPN as the SAN, they obtain a valid client-authentication certificate that impersonates that user. The certificate remains valid even if the target user resets their password, making it a durable credential for Pass-the-Certificate or PKINIT-based authentication.
