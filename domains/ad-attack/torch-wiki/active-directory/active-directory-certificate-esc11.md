...
type: technique
tags: [active-directory, adcs, certificates, esc11, exploitation, reference-import, windows]
phase: exploitation
...
ESC11 abuses an AD CS configuration where the RPC-based `ICertPassage` (ICPR) interface does not enforce encryption for certificate requests. An attacker relays NTLM authentication over RPC to the CA using a modified Impacket `ntlmrelayx` or Certipy relay module, requesting a certificate on behalf of a coerced machine account. Because the transport encryption requirement is disabled, the relay succeeds and the attacker obtains a certificate usable for PKINIT authentication.
