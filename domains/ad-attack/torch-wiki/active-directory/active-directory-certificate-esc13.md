...
type: technique
tags: [active-directory, adcs, certificates, esc13, exploitation, reference-import, windows]
phase: exploitation
...
ESC13 abuses certificate templates whose issuance policy has an `msDS-OIDToGroupLink` that maps the policy OID to a privileged AD group, typically a Universal group. When an attacker enrolls in such a template, the issued certificate grants group membership in the linked group during Kerberos authentication via PKINIT. The AD KDC includes the linked group's SID in the PAC of the TGT, giving the authenticated user effective membership in the privileged group without any direct group assignment in the directory.
