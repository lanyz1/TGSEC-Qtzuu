...
type: technique
tags: [active-directory, adcs, certificates, esc14, exploitation, reference-import, windows]
phase: exploitation
...
ESC14 abuses write access to the `altSecurityIdentities` attribute on an AD account, which stores explicit certificate-to-account mappings used for authentication. An attacker with write access over this attribute on a target account adds a mapping pointing to a certificate they control, then authenticates to the domain using that certificate to impersonate the target account. This works because the `altSecurityIdentities` mapping takes precedence over UPN-based certificate binding when present.
