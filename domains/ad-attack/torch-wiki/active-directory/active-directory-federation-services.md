...
type: technique
tags: [active-directory, exploitation, federation, reference-import, saml, windows]
phase: exploitation
...
AD FS issues SAML tokens signed with a token-signing certificate, and any party that holds the private key for that certificate can forge arbitrary SAML assertions, a technique known as a Golden SAML attack. Attackers who compromise the AD FS server extract the DKM master key (stored in an AD contact object) or the token-signing key, then use it to mint SAML tokens asserting any identity including privileged cloud or SaaS roles. Because SAML tokens are self-contained and verified by signature rather than by session state, the forged tokens are valid even when MFA is enforced on the identity provider.
