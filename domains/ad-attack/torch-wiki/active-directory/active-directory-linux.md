...
type: technique
tags: [active-directory, enumeration, linux, reference-import, windows]
phase: enumeration
...
Linux machines joined to an AD domain store Kerberos tickets as CCACHE files in `/tmp` or in the kernel keyring, accessible to root or any process running as the ticket owner's UID. An attacker with local root access can copy CCACHE files from other users' sessions and reuse them by setting `KRB5CCNAME`, impersonating those users against domain services without knowing their passwords. Keytab files stored for service accounts or SSSD credential caches can also be harvested to perform offline password cracking or direct Kerberos authentication.
