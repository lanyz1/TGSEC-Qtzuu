...
type: technique
tags: [active-directory, exploitation, reference-import, windows]
phase: exploitation
...
By default, the `ms-DS-MachineAccountQuota` domain attribute allows any authenticated user to create up to 10 computer accounts in the domain. Attackers exploit this to create a computer account they fully control, which can then be used as a base for Resource-Based Constrained Delegation (RBCD) attacks, shadow credentials, or Kerberos relay scenarios. The attacker-created machine account has a known password and `msDS-CreatorSID` set to the creating user's SID, enabling later cleanup and re-creation without needing elevated privileges.
