...
type: technique
tags: [active-directory, credentials, lateral-movement, reference-import, sccm, windows]
...
SCCM manages software deployment to all enrolled clients with a Network Access Account (NAA) that is stored encrypted on client machines but retrievable by any local administrator via tools like SharpSCCM. Attackers who compromise an SCCM site server or gain `Full Administrator` rights in the SCCM console can deploy applications to any managed machine in the collection, achieving domain-wide code execution. SCCM also stores PXE boot credentials and client push installation accounts that are frequently over-privileged and reusable for lateral movement.
