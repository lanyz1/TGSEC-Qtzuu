...
type: technique
tags: [active-directory, coercion, exploitation, ntlm, reference-import, windows]
phase: exploitation
...
Authentication coercion forces a Windows machine to authenticate outbound to an attacker-controlled host using NTLM or Kerberos, by calling MS-RPC protocol functions that trigger a callback as the machine account (SYSTEM). Primitives like PetitPotam (MS-EFSRPC), PrinterBug (MS-RPRN), DFSCoerce (MS-DFSNM), and ShadowCoerce (MS-FSRVP) call legitimate RPC functions that instruct the server to connect to a specified path, sending the machine account's credential in the process. The captured or relayed machine account credential is then used for NTLM relay attacks, certificate enrollment (ESC8), or DCSync if the coerced machine is a Domain Controller.
