...
title: Internal - DCOM
type: technique
tags: [active-directory, dcom, lateral-movement, reference-import, windows]
...
DCOM allows remote instantiation of COM objects and invocation of their methods over the network using the MSRPC protocol, authenticated via Kerberos or NTLM. Attackers with valid credentials for a target machine exploit DCOM objects such as `MMC20.Application`, `ShellWindows`, and `ShellBrowserWindow` to execute arbitrary commands remotely without deploying any tools to the target. This technique works because these DCOM interfaces expose shell execution methods that are accessible to local administrators, and because DCOM traffic is often permitted through host firewalls as a legitimate management protocol.
