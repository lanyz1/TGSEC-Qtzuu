...
type: technique
tags: [active-directory, exploitation, kerberos, rbcd, reference-import, windows]
phase: exploitation
...
RBCD allows a resource owner to control which principals can delegate to their service, stored in the target resource's `msDS-AllowedToActOnBehalfOfOtherIdentity` attribute. An attacker with `GenericWrite` or `WriteDACL` over a computer account writes a controlled machine account's SID into the target's RBCD attribute, then uses S4U2self and S4U2proxy from the controlled machine to obtain a service ticket impersonating any user (including domain admins) for the target. Because `MachineAccountQuota` allows any domain user to create computer accounts, this attack chain frequently requires only a low-privilege domain account.
