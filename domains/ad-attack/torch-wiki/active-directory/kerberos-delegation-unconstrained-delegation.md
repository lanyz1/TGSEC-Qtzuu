...
Unconstrained delegation allows a service to impersonate any authenticating user for any service, because the user's full TGT is forwarded to and stored in the memory of the delegating host when they authenticate. An attacker who compromises a server with unconstrained delegation uses coercion primitives (PrinterBug, PetitPotam) to force Domain Controller machine accounts to authenticate to the compromised server, then captures their TGTs from memory with Mimikatz `sekurlsa::tickets`. With a DC machine account's TGT, the attacker can perform a DCSync or access any service in the domain as that DC.
