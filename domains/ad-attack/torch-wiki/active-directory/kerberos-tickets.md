...
type: technique
tags: [active-directory, credentials, golden-ticket, kerberos, reference-import, silver-ticket, windows]
...
Kerberos tickets (TGTs and service tickets) are encrypted data structures that authenticate users to services; they have a configurable lifetime (typically 8-12 hours) and are stored in memory (on Windows, managed by the LSA) or as CCACHE files on Linux. Attackers steal tickets from memory using Mimikatz `sekurlsa::tickets` or Rubeus `dump`, then inject them into their own session via Pass-the-Ticket to impersonate the ticket owner. Golden Tickets (forged TGTs signed with the krbtgt hash) and Silver Tickets (forged service tickets signed with a service account hash) are attacker-crafted tickets that grant persistent, undetectable authentication for the ticket's lifetime.
