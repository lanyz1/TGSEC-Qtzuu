...
type: technique
tags: [active-directory, cve, exploitation, kerberos, reference-import, windows]
phase: exploitation
...
MS14-068 is a critical Kerberos vulnerability where the Windows KDC fails to validate the checksum type in a PAC (Privilege Attribute Certificate) within a TGT. An attacker provides a forged PAC that claims Domain Admin group membership, signed with an MD5 checksum using the user's own password hash, and the KDC accepts it because it does not enforce the use of strong checksum algorithms. The result is a valid TGT with Domain Admin privileges for any authenticated domain user, enabling immediate full domain compromise on unpatched Domain Controllers.
