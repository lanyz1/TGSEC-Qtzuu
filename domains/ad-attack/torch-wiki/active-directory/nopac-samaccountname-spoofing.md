...
NoPAC (CVE-2021-42278 and CVE-2021-42287) exploits two AD vulnerabilities: the KDC does not validate that a computer account's `sAMAccountName` is unique from existing machine accounts, and during S4U2Self the KDC appends `$` to the name to find the machine account if not found initially. An attacker creates a computer account (via `MachineAccountQuota`), renames it to match an existing DC's `sAMAccountName` without the trailing `$`, requests a TGT, renames the account again, then uses S4U2Self to obtain a service ticket impersonating a Domain Admin, as the KDC resolves the `$`-suffixed name to the real DC. This results in a Domain Admin service ticket without requiring any prior privileged access.
