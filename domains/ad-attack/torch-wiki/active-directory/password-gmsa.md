...
title: Password - GMSA
type: technique
tags: [active-directory, credentials, gmsa, reference-import, windows]
...
Group Managed Service Accounts (GMSAs) have their passwords automatically managed by AD and rotated every 30 days, but the password can be retrieved by any principal listed in the gMSA's `PrincipalsAllowedToRetrieveManagedPassword` attribute. An attacker who compromises an account with read access to a gMSA's password retrieves it using `Get-ADServiceAccount -Properties msDS-ManagedPassword` or DSInternals tools, obtaining the 256-byte NT hash for Pass-the-Hash. Since gMSAs are often used for high-privilege services (IIS application pools, scheduled tasks), their compromise frequently provides direct lateral movement to critical infrastructure.
