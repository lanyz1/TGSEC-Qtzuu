...
title: Password - LAPS
type: technique
tags: [active-directory, credentials, laps, reference-import, windows]
...
LAPS (Local Administrator Password Solution) manages unique, randomly generated local administrator passwords for each domain-joined computer, storing them in the `ms-Mcs-AdmPwd` attribute of the computer object in AD. An attacker with `ReadProperty` rights on the `ms-Mcs-AdmPwd` attribute (or the newer `msLAPS-Password` for LAPS v2) can read the plaintext local administrator password for that machine via a simple LDAP query. Identifying which principals have read access to LAPS attributes and which machines are in scope often provides immediate access to systems where lateral movement is needed.
