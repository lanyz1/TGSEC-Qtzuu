...
type: technique
tags: [active-directory, certificates, exploitation, reference-import, shadow-credentials, windows]
phase: exploitation
...
Shadow Credentials exploits the `msDS-KeyCredentialLink` attribute on AD user and computer objects, which stores public-key credentials used for Kerberos PKINIT authentication (Windows Hello for Business). An attacker with `GenericWrite` or `WriteProperty` over a target account adds their own public key to this attribute using Whisker or certipy, then authenticates via PKINIT using the matching private key to obtain a TGT for the target account. Because `msDS-KeyCredentialLink` modification does not require the target's current credential and survives password resets, this provides persistent access to the account.
