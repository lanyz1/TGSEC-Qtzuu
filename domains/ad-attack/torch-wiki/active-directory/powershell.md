...
Technical reference for **Powershell** collected from InternalAllTheThings during an internal/cloud assessment ingest.

## How it works

PowerShell is a primary attack and post-exploitation tool because it provides direct access to the Windows API, .NET runtime, COM objects, and WMI, all from a scripting interface that is built into every modern Windows system. Attackers bypass the Execution Policy using `powershell.exe -ExecutionPolicy Bypass`, encode commands in Base64 to avoid string-based detection, and use Constrained Language Mode bypasses or AMSI evasion to run scripts in locked-down environments. PowerShell remoting (`Enter-PSSession`, `Invoke-Command`) enables agentless lateral movement to any host where the current user has remote management rights.
