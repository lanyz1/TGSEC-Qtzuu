...
type: technique
tags: [active-directory, enumeration, kerberos, password-cracking, reference-import, windows]
phase: enumeration
...
AS-REP Roasting targets user accounts with Kerberos pre-authentication disabled (`UF_DONT_REQUIRE_PREAUTH`), a setting that allows anyone to request an AS-REP for the account without first proving knowledge of the password. The KDC encrypts part of the AS-REP with the user's NTLM hash, and the attacker can request this structure anonymously then crack the encrypted portion offline to recover the user's password. Accounts with pre-authentication disabled are often service accounts or legacy accounts, and their passwords are frequently weaker than interactive user accounts.
