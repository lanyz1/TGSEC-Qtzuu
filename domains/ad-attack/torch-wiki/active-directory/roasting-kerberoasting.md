...
type: technique
tags: [active-directory, kerberoasting, kerberos, password-cracking, reference-import, windows]
phase: enumeration
...
Kerberoasting requests service tickets for accounts that have Service Principal Names (SPNs) registered; any authenticated domain user can request these tickets. The service ticket is encrypted with the service account's NTLM hash, and the attacker extracts the ticket from memory then cracks it offline with hashcat to recover the service account's password. Service accounts are high-value targets because they often have excessive privileges and infrequently changed passwords, making the cracked hash useful for lateral movement to the services they run.
...
- [[john]]
- [[netexec]]
- Also uses (no dedicated page yet): Rubeus, PowerView, PowerSploit
