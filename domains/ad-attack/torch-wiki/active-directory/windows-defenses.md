...
Technical reference for **Windows - Defenses** collected from InternalAllTheThings during an internal/cloud assessment ingest.

## How it works

Windows security defenses include AppLocker (application whitelisting), UAC (privilege elevation controls), AMSI (script content scanning), Constrained Language Mode (PowerShell restrictions), Credential Guard (LSASS isolation via virtualization), Protected Process Light (PPL for LSASS), and ETW (Event Tracing for Windows) for security telemetry. Attackers enumerate which defenses are active before selecting evasion techniques, since each defense has specific bypass paths; for example, PPL-protected LSASS blocks Mimikatz but can be bypassed with a signed driver or kernel exploit. Understanding all active defenses as a combined control set is essential before choosing post-exploitation technique sequences.
