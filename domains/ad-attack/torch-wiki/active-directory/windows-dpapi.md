...
title: Windows - DPAPI
type: technique
tags: [credentials, dpapi, reference-import, windows]
...
DPAPI (Data Protection API) encrypts secrets like browser-saved passwords, Windows Credential Manager entries, Wi-Fi PSKs, and private keys using a masterkey derived from the user's login password and a machine-specific secret. An attacker with access to the user's profile can decrypt DPAPI blobs offline by combining the encrypted masterkey with the user's NT hash or plaintext password, using tools like SharpDPAPI or Mimikatz `dpapi::*`. Domain-joined machine DPAPI masterkeys can also be decrypted using the domain backup key (stored on the DC), meaning Domain Admin access enables decryption of any user's DPAPI secrets without knowing their password.
...
[diagram: HEKATOMB DPAPI credential extraction](https://github.com/ProcessusT/HEKATOMB/raw/main/.assets/github1.png)
