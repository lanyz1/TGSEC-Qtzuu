...
type: technique
tags: [persistence, reference-import, windows]
...
Technical reference for **Windows - Persistence** collected from InternalAllTheThings during an internal/cloud assessment ingest.

## How it works

Windows persistence mechanisms maintain attacker access across reboots and logoffs by registering code to run automatically via registry run keys (`HKCU\Software\Microsoft\Windows\CurrentVersion\Run`), startup folder shortcuts, scheduled tasks, Windows services, IIS modules, COM hijacking, and DLL side-loading. User-level persistence mechanisms (run keys, startup folder, scheduled tasks under user context) survive user logout but are wiped on account deletion; machine-level persistence (services, system scheduled tasks) survives user changes but requires elevated privileges to install. Persistence location choice is driven by the required privilege level, stealth requirements, and the security tooling monitoring specific autorun locations.
