...
Technical reference for **AWS - Enumerate** collected from InternalAllTheThings during an internal/cloud assessment ingest.

## How it works

AWS enumeration maps the attack surface of an account by listing IAM users, roles, groups, policies, EC2 instances, S3 buckets, Lambda functions, and other resources across all regions. Tools like ScoutSuite, Pacu, and `enumerate-iam` automate this by issuing read-only API calls with captured credentials and identifying misconfigurations such as overly permissive IAM policies, public S3 buckets, and exposed metadata endpoints. The goal is to identify the permissions available to the current identity and locate privilege escalation paths or sensitive data stores.
