...
type: technique
tags: [aws, cloud, exploitation, reference-import, ssrf]
phase: exploitation
...
The AWS Instance Metadata Service (IMDS) at `169.254.169.254` exposes temporary IAM credentials assigned to the EC2 instance role when accessed from within the instance. An SSRF vulnerability in a web application running on EC2 allows an attacker to proxy requests to the metadata endpoint and retrieve the role credentials from `/latest/meta-data/iam/security-credentials/<role-name>`. IMDSv1 requires no authentication token and is therefore trivially exploitable via SSRF; IMDSv2 mitigates this by requiring a PUT-based preflight request to obtain a session token before credential retrieval.
...
3. Extract the role's temporary keys : `http://169.254.169.254/latest/meta-data/iam/security-credentials/<IAM_USER_ROLE_HERE>/`
