...
type: technique
tags: [aws, cloud, ec2, exploitation, reference-import]
phase: exploitation
...
Technical reference for **AWS - Service - EC2** collected from InternalAllTheThings during an internal/cloud assessment ingest.

## How it works

EC2 instances commonly hold secrets in user data (startup scripts), environment variables, and on attached EBS volumes, all of which are accessible to anyone with the instance's IAM role or with `ec2:DescribeInstances` access. Attackers snapshot a target EBS volume, attach it to an attacker-controlled instance, and mount it to extract data without needing operating system credentials. EC2 instance profiles grant temporary AWS credentials via the metadata service, making any SSRF or RCE vulnerability on an EC2 instance a vector for cloud lateral movement.
