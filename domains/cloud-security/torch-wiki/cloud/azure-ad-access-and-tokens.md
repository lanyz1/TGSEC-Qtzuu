...
type: technique
tags: [active-directory, azure, cloud, credentials, oauth, reference-import]
...
Azure AD issues OAuth 2.0 access tokens and refresh tokens to applications and users authenticating against Microsoft services; an attacker who captures a refresh token can silently request new access tokens for any resource the token's scope permits. The Microsoft Graph API and Azure Management API accept Bearer tokens, so stealing a valid token from browser storage, memory, or a proxy gives immediate access to cloud resources without re-authentication. Tokens obtained via phishing device-code flows, OAuth consent grants, or by reading cached MSAL token stores are particularly powerful because they include refresh tokens that persist across session termination.
