...
type: technique
tags: [active-directory, azure, cloud, lateral-movement, reference-import]
...
Technical reference for **Azure AD - AD Connect and Cloud Sync** collected from InternalAllTheThings during an internal/cloud assessment ingest.

## How it works

Azure AD Connect synchronizes on-premises Active Directory to Azure AD using a service account with replication privileges; an attacker who compromises this account or the AD Connect server can extract cleartext credentials for all synchronized accounts or perform a Password Hash Sync (PHS) attack to dump Azure AD hashes. For Pass-Through Authentication (PTA), injecting a DLL into the PTA agent intercepts authentication requests and captures credentials in cleartext. For federated environments, compromising the ADFS server's token-signing certificate enables Golden SAML attacks, granting access to any federated application as any user.
