...
title: Azure AD - IAM
type: technique
tags: [active-directory, azure, cloud, iam, privilege-escalation, reference-import]
...
Azure's IAM hierarchy flows from Root Management Group down through Management Groups, Subscriptions, Resource Groups, and individual Resources; role assignments at any level inherit downward. Attackers enumerate role assignments at each scope to find principals with excessive permissions such as `Owner`, `Contributor`, or `User Access Administrator`, which allow creating new resources, modifying IAM, or escalating to Global Administrator via role assignment. Service principals and managed identities with subscription-level permissions are high-value targets because they often hold long-lived credentials or implicit cloud-wide access.
