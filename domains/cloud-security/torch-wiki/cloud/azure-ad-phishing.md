...
type: technique
tags: [active-directory, azure, cloud, initial-access, phishing, reference-import]
phase: exploitation
...
Azure AD phishing abuses OAuth consent flows and device authorization flows to steal access tokens without capturing passwords. In an illicit consent grant attack, the attacker registers a malicious Azure application and sends a crafted consent URL; when the victim clicks it, the app is granted delegated permissions to their account, giving the attacker API access tokens for Microsoft Graph or other resources. In a device code phishing attack, the attacker initiates a device code authentication flow and tricks the victim into entering the code, which hands the attacker a fully authorized refresh token for the victim's session.
