...
type: technique
tags: [azure, cloud, container, exploitation, reference-import]
phase: exploitation
...
Azure Container Registry (ACR) stores Docker images used by production workloads; an attacker with `AcrPull` or `AcrPush` permissions, or who obtains an ACR access token, can pull production images to extract hardcoded secrets, environment variables, and application configuration baked into image layers. ACR admin credentials (username and password) stored in automation or configuration files provide full registry access. Pushing a modified image with a backdoor into the registry can compromise any workload that pulls and deploys the image.
