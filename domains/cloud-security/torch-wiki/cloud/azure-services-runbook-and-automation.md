...
type: technique
tags: [azure, cloud, lateral-movement, persistence, reference-import]
...
Azure Automation runbooks execute PowerShell or Python code in Azure with the permissions of the associated Automation Account's managed identity or Run As account, which often holds `Contributor` or higher rights on the subscription. Attackers who gain `Automation Contributor` or higher access create a new runbook, publish it, and run it to execute arbitrary code in the Azure management plane, effectively escalating to whatever permissions the Automation Account holds. The Run As account's certificate-based credentials are extractable from the Automation Account and can be used outside of Azure to authenticate as the associated service principal.
