...
**Pattern:** Disk image contains N puzzle-piece PNGs carved out by `foremost` or `scalpel`. Stitch all pieces horizontally with ImageMagick `convert +append`, then feed the strip to the `gaps` jigsaw solver ([upstream-repo]) with the known piece size (often stored in EXIF) to auto-reassemble.
