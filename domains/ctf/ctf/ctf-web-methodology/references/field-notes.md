...
## LFI → Source Code → Route Discovery

When you have LFI (Local File Inclusion) that lets you read application source code, **stop brute-forcing paths** and switch to source-code-driven route extraction. This is faster, more accurate, and avoids wasting turns on 404s.

**Step 1: Map the include tree**
```bash
# Read entry point (index.php / app.py / server.js)
# Extract all require/include/import statements
grep -oP "require(_once)?\s*['\"]([^'\"]+)['\"]" index.php
grep -oP "include(_once)?\s*['\"]([^'\"]+)['\"]" dashboard.php
# Follow each include recursively — hidden endpoints live in include chains
```

**Step 2: Extract all routes/endpoints from source**
```bash
# PHP: grep for routing patterns
grep -rn 'header.*Location\|action.*=\|POST\|GET\|ajax\|fetch(' *.php
grep -rn '\$_GET\|\$_POST\|\$_REQUEST' *.php

# JS/AJAX in HTML: grep for API calls
grep -oP "url:\s*['\"]([^'\"]+)['\"]" dashboard.php
grep -oP "fetch\(['\"]([^'\"]+)['\"]" dashboard.php
grep -oP "action=['\"]([^'\"]+)['\"]" dashboard.php
```

**Step 3: Identify hidden parameters**
```bash
# Find action/operation switches inside PHP files
grep -n 'switch.*action\|if.*action.*==\|case.*approve\|case.*delete' *.php
# These reveal endpoints that don't have their own file — they're
# parameters on existing endpoints (e.g., POST /dashboard.php?action=approve)
```

**Key insight:** In PHP apps especially, a single file (dashboard.php) often handles 5+ different actions via `$_POST['action']` or `$_GET['do']` switch statements. These are invisible to directory brute-forcing. Only source code analysis reveals them.

**Common patterns to search for:**
| Language | Pattern | Reveals |
|----------|---------|---------|
| PHP | `$_POST['action']`, `switch($action)` | Hidden same-page actions |
| PHP | `require_once`, `include` | Hidden files in include chain |
| Python/Flask | `@app.route`, `url_for` | All registered routes |
| Node/Express | `router.get`, `router.post` | All registered routes |
| Java/Spring | `@RequestMapping`, `@PostMapping` | All controller endpoints |
| HTML/JS | `$.ajax`, `fetch(`, `action=` | Frontend-called endpoints |
