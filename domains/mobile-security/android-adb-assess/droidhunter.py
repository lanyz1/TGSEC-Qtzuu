...
"""
╔══════════════════════════════════════════════════════════════════╗
║          android-adb-assess — Advanced Android Pentesting Tool          ║
║          Author : TGSEC                                     ║
║          Contact: Instagram @TGSEC                          ║
...
def print_banner():
    """Print the animated android-adb-assess banner with live status."""
...
MENU_OPTIONS = [
    ...
    ("17", "❔", "About",                   "About android-adb-assess"),
    ("0",  "🚪", "Exit",                    "Exit android-adb-assess"),
]
...
def handle_report_generator():
    ...
    if fmt in ("html", "both"):
        out = Prompt.ask("[cyan]HTML output filename[/]", default="android-adb-assess_report.html")
        report_generator.generate_html_report(data, out)

    if fmt in ("json", "both"):
        out = Prompt.ask("[cyan]JSON output filename[/]", default="android-adb-assess_report.json")
...
def handle_about():
    about = Panel(
        ...
        title="[bold]About android-adb-assess[/]",
        border_style="magenta",
        padding=(0, 4),
    )
    console.print(about)
...
def interactive_mode():
    ...
    console.print(Panel(
        ...
        "[white]android-adb-assess is designed for authorized security testing ONLY.\n"
    ...
    while True:
        ...
        choice = Prompt.ask("\n[bold cyan]android-adb-assess ▶[/]", choices=valid_choices, show_choices=False)

        if choice == "0":
            console.print("\n[bold magenta]👻 Exiting android-adb-assess. Stay ethical.[/]\n")
            sys.exit(0)
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="android-adb-assess",
        description=f"👻 android-adb-assess v{VERSION} — Advanced Android Pentesting Tool by {AUTHOR}",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python3 android-adb-assess.py --interactive
  python3 android-adb-assess.py --apk app.apk --report html
  ...
  python3 android-adb-assess.py --payload reverse_tcp --lhost 10.0.0.1 --lport 4444
  python3 android-adb-assess.py --device ABC123 --exploit deep-link --pkg com.example --scheme myapp
...
def cli_mode(args):
    ...
    if args.report:
        ...
        if args.report in ("html", "both"):
            out = args.report_out or "android-adb-assess_report.html"
            report_generator.generate_html_report(data, out)
        if args.report in ("json", "both"):
            out = args.report_out or "android-adb-assess_report.json"
            report_generator.generate_json_report(data, out)
...
if __name__ == "__main__":
    try:
        ...
        console.print("\n\n[bold magenta]👻 android-adb-assess interrupted. Stay ethical.[/]\n")
        sys.exit(0)
