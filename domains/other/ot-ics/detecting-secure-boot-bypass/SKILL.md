...
- CHIPSEC (run from a live USB or controlled host; loads a kernel driver):
  ```bash
  pip install chipsec        # or build from [upstream-repo]
  ```
...
| Tool | Purpose | Source |
|------|---------|--------|
| mokutil | Secure Boot state and enrolled keys (Linux) | [upstream-repo] |
| efitools (efi-readvar) | Dump PK/KEK/db/dbx | https://git.kernel.org/pub/scm/linux/kernel/git/jejb/efitools.git |
| dbxtool | Inspect and apply dbx updates | [upstream-repo] |
| CHIPSEC | Firmware / Secure Boot variable assessment | [upstream-repo] |
| sbsigntool / pesign | EFI binary signature verification | [upstream-repo] |
