...
type: technique
tags: [active-directory, credentials, ntlm, password-cracking, reference-import, windows]
...
NTLM hashes are captured from network traffic by poisoning name resolution protocols (LLMNR, NBT-NS, mDNS) with tools like Responder, which causes victim machines to send NetNTLM challenge-response authentication to an attacker-controlled listener. The captured NetNTLMv1/v2 hashes are not directly usable for Pass-the-Hash but can be cracked offline with hashcat or John the Ripper using wordlist and rule-based attacks to recover the plaintext password. The `LmCompatibilityLevel` registry setting determines which hash format is sent, with lower values producing weaker LM or NTLMv1 hashes that crack more quickly.
