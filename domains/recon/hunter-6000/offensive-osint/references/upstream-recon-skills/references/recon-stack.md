...
| **SecLists** | `[upstream-repo]` | Massive collection. Subdomains: `Discovery/DNS/subdomains-top1million-110000.txt`. Content: `Discovery/Web-Content/`. |
| **jhaddix all.txt** | `https://gist.github.com/jhaddix/86a06c5dc309d08580a018c66354a056` | Long-running curated list. |
| **OneListForAll** | `[upstream-repo]` | Aggregated; very large (millions). |
| **dirsearch wordlists** | `[upstream-repo]` | Bundled with the tool. |
| **raft-large-words.txt** | inside SecLists `Discovery/Web-Content/raft-large-words.txt` | Time-tested content wordlist. |
| **bo0om wordlist** | `[upstream-repo]` | Russian-language-aware. |
| **commonspeak2** | `[upstream-repo]` | Generated from BigQuery commit data. |
| **fuzzdb** | `[upstream-repo]` | Fuzzing payloads + wordlists. |
| **PayloadsAllTheThings** | `[upstream-repo]` | Per-vuln-class payloads (less for enum, more for follow-on). |
...
- [theHarvester]([upstream-repo]) — subdomain, email, metadata.
- [Recon-ng]([upstream-repo]) — web recon framework.
- [Amass]([upstream-repo]) / [Subfinder]([upstream-repo]) — passive subdomain.
...
# Or native install: [upstream-repo]
