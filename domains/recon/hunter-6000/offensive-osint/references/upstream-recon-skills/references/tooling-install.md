...
git clone [upstream-repo] && cd masscan && make
```
...
git clone [upstream-repo] && cd LinkFinder && pip install -r requirements.txt
...
git clone [upstream-repo] && cd cloud_enum && pip install -r requirements.txt

# S3Scanner
pip install s3scanner

# CloudSploit
git clone [upstream-repo] && cd cloudsploit && npm install
```
...
git clone [upstream-repo] && cd reconftw && ./install.sh

# Axiom (distributed recon on cloud nodes)
git clone [upstream-repo] && cd axiom && ./interact/axiom-configure
```
