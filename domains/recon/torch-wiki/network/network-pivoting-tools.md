...
Network pivoting tools create tunneled connections through compromised hosts to reach isolated network segments, using a variety of transport protocols (HTTPS, WebSockets, ICMP) to blend with normal traffic and bypass firewall rules. Tools like Chisel, Ligolo-ng, and rpivot support SOCKS4/5 proxy creation over encrypted tunnels, allowing transparent proxying of attacker tooling through multiple hop points. The choice of tool is driven by the target environment's transport restrictions, available runtimes on the pivot host, and whether the connection must be outbound-only (useful when inbound connections to the pivot are blocked).
