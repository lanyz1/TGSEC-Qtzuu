...
type: technique
tags: [cicd, cloud, linux, recon, reference-import, supply-chain, windows]
phase: recon
...
Build system files (`Makefile`, `package.json` scripts, `Gemfile`, `setup.py`, `pom.xml`) run arbitrary code during dependency installation and build steps, making them viable injection points in any CI/CD pipeline that processes them. Attackers who can modify these files (via pull request, fork, or SCM access) inject shell commands into build hooks, `postinstall` scripts, or custom build targets that execute during the pipeline's dependency resolution phase. Because these injections are CI-system agnostic, they work across GitHub Actions, GitLab CI, CircleCI, and Jenkins without modification.
