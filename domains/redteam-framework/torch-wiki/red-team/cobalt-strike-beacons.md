...
Technical reference for **Cobalt Strike - Beacons** collected from InternalAllTheThings during an internal/cloud assessment ingest.

## How it works

Cobalt Strike beacons are shellcode-based implants that communicate with a team server over HTTP/S, DNS, or SMB named pipes using a malleable C2 profile that shapes network traffic to blend with legitimate services. DNS beacons resolve subdomains delegated to the attacker's NS records, making outbound communication appear as normal DNS queries to most firewalls. Beacons operate in a sleep-poll loop, periodically checking in for tasks, which reduces their network signature compared to persistent connection C2 channels.
