...
title: Cobalt Strike
type: technique
tags: [c2, cobalt-strike, reference-import, windows]
...
Cobalt Strike is a commercial adversary simulation tool built around a client-server architecture: the team server hosts the C2 infrastructure and generates payloads, while operators connect via the Cobalt Strike client to manage active beacons. Beacons are deployed as shellcode stagers that fetch a reflectively-loaded DLL (the stage) over HTTP/S, DNS, or SMB, then establish a polling communication channel with the team server using a malleable C2 profile that shapes traffic to mimic legitimate services. The platform provides post-exploitation modules for credential dumping, lateral movement, pivoting, and privilege escalation that chain naturally with beacon sessions.
