...
title: OPSEC
type: technique
tags: [evasion, linux, opsec, reference-import, windows]
...
Technical reference for **OPSEC** collected from InternalAllTheThings during an internal/cloud assessment ingest.

## How it works

Red team OPSEC (Operational Security) focuses on avoiding detection by blue teams and security tooling throughout an engagement by minimizing attacker footprint in logs, network traffic, and endpoint artifacts. Infrastructure OPSEC practices include using generic DNS names, wildcard certificates, non-default C2 certificates, and redirectors to prevent attribution and blue team takedown of active C2 infrastructure. Behavioral OPSEC avoids noisy commands like `whoami` after injection, uses guardrails to prevent payload execution outside the target environment, and shapes beacon behavior to match normal user and process activity patterns.
