...
title: Phishing
type: technique
tags: [initial-access, phishing, reference-import, web, windows]
phase: exploitation
date_created: 2026-05-13
date_updated: 2026-07-02
sources: [InternalAllTheThings, mrdox-bitb, quishing-research]
---
...
Phishing attacks deliver malicious content (credential harvesting pages, weaponized attachments, malware links) by impersonating trusted senders or organizations to manipulate targets into taking an action they believe is legitimate. Spear-phishing targets specific individuals with personalized content derived from OSINT, increasing success rates compared to mass phishing campaigns. Modern phishing campaigns use techniques like adversary-in-the-middle (AiTM) proxy phishing (via Evilginx2 or Modlishka) to capture session cookies even when MFA is enabled, bypassing the password and OTP layer entirely.
...
Render a fake browser popup *inside* the page (HTML/CSS that mimics the OS window chrome + address bar showing a legit URL like `https://login.microsoftonline.com`) for a fake SSO prompt. The victim sees a trusted URL that is just text in the attacker page; credentials post to the attacker. Defeats the "check the URL" habit because the real browser URL bar is unchanged (the popup is fake). Combine with a convincing parent page.

- Tooling: `mrd0x/BITB` template kit; tailor the chrome to the victim OS/browser (Windows/macOS, light/dark).
- Tell for defenders/victims: the "popup" cannot leave the page bounds, has no real OS window controls, and dev-tools show it is a `<div>`.
- Stronger when chained with [[#Evilginx]] / AiTM so the fake window proxies a real session (captures MFA cookies), not just static creds.

## Quishing (QR-code phishing)

Embed the phishing link in a QR code (image in the email/PDF/poster) instead of a clickable link. Bypasses email URL scanners/sandboxes (many do not OCR/parse QR images) and pushes the victim onto a **mobile device** - typically outside EDR/proxy, with a smaller screen that hides URL tells and where MFA approvals happen.

- Delivery: QR in a PDF attachment ("scan to view secure document"), fake MFA-reset notice, parking/invoice lures.
- Pair with an AiTM landing page so the mobile session yields session cookies; mobiles often auto-approve push MFA.
- Defence: QR-aware mail filtering, block new/lookalike domains, number-matching MFA, user training that QR == URL.
